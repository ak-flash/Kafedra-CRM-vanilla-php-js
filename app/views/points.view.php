...
<!-- Content Header (Page header) -->
      <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-8">
              <h1><b>Рассчёт среднего балла студента за семестр</b></h1>
              </div>
            <div class="col-sm-4">
               Журнал группы: &nbsp;&nbsp;&nbsp;<input id="the-file-input" class="btn btn-danger" type="file" onchange="renderImage(this.files[0]);$('#the-file-input').removeClass('btn-danger');$('#the-file-input').addClass('btn-success');">
              </div>
          </div>
        </div><!-- /.container-fluid -->
      </section>
...
while (i < 38) {
	...
	if(document.getElementById("check"+i).checked){points_i=points_i+parseInt($('#i'+i).val());c_i++;} else {points=points+parseInt($('#i'+i).val());c++;}
	} 
...
if(val=='plus'){document.getElementById("s"+i).style.width=(parseInt(document.getElementById("s"+i).style.width)+1)+"px";} else {document.getElementById("s"+i).style.width=(parseInt(document.getElementById("s"+i).style.width)-1)+"px";
}
i++;
}
